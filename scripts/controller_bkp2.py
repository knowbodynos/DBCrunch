...
def islimitreached(controllerpath,querylimit,niters):
    if querylimit==None:
        return False;
    else:
        if niters==1:
            ...
            ntot=eval(subprocess.Popen("echo \"$(cat $(find "+controllerpath+"/jobs/ -type f -name '*.stat' 2>/dev/null | sed 's/stat/docs/g') 2>/dev/null | wc -l)+$(cat "+controllerpath+"/jobs/*.log 2>/dev/null | grep 'CPUTime' | wc -l)\" | bc | head -c -1",shell=True,stdout=subprocess.PIPE,preexec_fn=default_sigpipe).communicate()[0]);
...
def reloadskippedjobs(modname,controllername,controllerpath,querystatefilename,basecollection,dbindexes,niters):
    try:
        skippedjobs=[];
...
def writejobfile(modname,dbpush,markdone,writestats,writestorage,jobname,jobstepnames,controllerpath,controllername,writemode,partitiontimelimit,buffertimelimit,partition,nnodes,ncores,memoryperstep,mongouri,scriptpath,scriptlanguage,scriptcommand,scriptflags,scriptext,basecollection,dbindexes,nthreadsfield,niters,docbatches):
    ...
    jobstring+="niters="+str(niters)+"\n";
    if niters>1:
        jobstring+="dbindexes=\""+str(dbindexes).replace(" ","")+"\"\n";
    ...
    for i in range(nbatches):
        ...
        if niters==1:
            jobstring+="docs["+str(i)+"]=\""+formatinput(docbatches[i][0])+"\"\n";#,scriptlanguage)+"\"\n";
            if nthreadsfield=="":
                jobstring+="njobstepthreads["+str(i)+"]=1\n";
            else:
                jobstring+="njobstepthreads["+str(i)+"]="+str(docbatches[i][0][nthreadsfield])+"\n";
        elif niters>1:
            ...
            if nthreadsfield=="":
                ...
                jobstring+="njobstepthreads["+str(i)+"]="+str(max([x[nthreadsfield] for x in docbatches[i]]))+"\n";   
    jobstring+="\n";
    #jobstring+="nbatches=0\n";
    #jobstring+="njobthreads=0\n";
    #jobstring+="for i in {0.."+str(nbatches-1)+"}\n";
    #jobstring+="do\n";
    #jobstring+="    nbatches=$((${nbatches}+1))\n";
    ...
    #    jobstring+="        echo -e \"$pendlicenseheader\n$((${npendlicenses}-${nbatches})),$((${npendsublicenses}-${njobthreads}))\" >"+pendlicensestatefile+"\n";
    #    jobstring+="    else\n";
    ...
    if niters>1:
        jobstring+=scriptcommand+" "+scriptflags+" \"${modulescriptpath}/"+modname+scriptext+"\" \"${workpath}\" \"${jobstepnames[${i}]}\" \"${dbindexes}\" > \"${workpath}/${jobstepnames[${i}]}.log\" &\n";
    elif niters==1:
    ...
    jobstring+="                srun -N 1 -n 1 --exclusive -J \"stats_${jobstepnames[${i}]}\" --mem-per-cpu=\""+str(memoryperstep/1000000)+"M\" python \"${scriptpath}/stats.py\" \"${modname}\" \"${controllername}\" \"${basecollection}\" \"${controllerpath}\" \"${jobstepnames[${i}]}\" \"${dbpush}\" \"${markdone}\" \"${writestats}\" \"${writestorage}\" \"${niters}\" \"${cputime}\" \"${maxrss}\" \"${maxvmsize}\" >> \"${workpath}/${jobstepnames[${i}]}.log\" &\n";# > ${workpath}/${jobname}.log\n";
...
def waitforslots(licensestream,username,modname,controllername,controllerpath,querystatefilename,basecollection,maxjobcount,modlist,modulesdirpath,softwarestatefile,scriptpath,scriptlanguage,maxthreads,starttime,controllerbuffertimelimit,statusstatefile,sleeptime,partitions,dbindexes,niters):
    ...
    reloadskippedjobs(modname,controllername,controllerpath,querystatefilename,basecollection,dbindexes,niters);
...
def doaction(batchcounter,stepcounter,inputdoc,docbatch,nthreadsfield,licensestream,username,maxjobcount,modlist,modulesdirpath,softwarestatefile,scriptpath,scriptlanguage,starttime,controllerbuffertimelimit,statusstatefile,sleeptime,partitions,resourcesstatefile,scriptmemorylimit,maxstepcount,modname,controllername,dbindexes,dbpush,markdone,writestats,writestorage,controllerpath,writemode,mongouri,scriptcommand,scriptflags,scriptext,querystatefilename,basecollection,niters):
    ...
    writejobfile(modname,dbpush,markdone,writestats,writestorage,jobname,jobstepnames,controllerpath,controllername,writemode,partitiontimelimit,buffertimelimit,partition,nnodes,ncores,memoryperstep,mongouri,scriptpath,scriptlanguage,scriptcommand,scriptflags,scriptext,basecollection,dbindexes,nthreadsfield,niters,docbatchwrite);
    #Submit job file
    if nthreadsfield!="":

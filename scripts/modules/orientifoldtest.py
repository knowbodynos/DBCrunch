...
def main_one(polyid, geonum, trinum, invnum, h11, h21, invol, basisinds, dresverts, sr, rwmat):
    ...
    if q > k:
        ...
        output.update({"GENS":py2mat(prx.gens())})
        ...
        output.update({origcykey:py2mat(cypoly)})
        nterms1 = len(cypoly.dict().keys())
        cypoly = symm_poly_swap(cypoly, invol, prx)
        output.update({symcykey:py2mat(cypoly)})
        ...
        output.update({"GENS":py2mat(pr.gens())})
    ...
    for i in range(len(oplanes)):
        ...
        fsetred = oplanesred[i][oidealkey]
        for j in range(len(fset)):
    #        w = prn(fset[j])
    #        w = w(pt)
    #        fset[j] = py2mat(prn(w))
            fset[j] = py2mat(fset[j])
        oplanes[i][oidealkey] = fset
        for j in range(len(fsetred)):
    #        w = prn(fsetred[j])
    #        w = w(pt)
    #        fsetred[j] = py2mat(prn(w))
            fsetred[j] = py2mat(fsetred[j])
        oplanesred[i][oidealkey] = fsetred
        oplanesall.append({origkey:oplanes[i],redkey:oplanesred[i]})
    ...
    output.update({oplaneskey:oplanesall})
...
newoplanesall = [x for x in output["OPLANES"] if x["RED"]["OIDEAL"]!=x["ORIG"]["OIDEAL"]]
if len(newoplanesall)>0:
    output.update({"OPLANES":newoplanesall})

...
def writejobfile(modname,dbpush,markdone,writestats,jobname,jobstepnames,controllerpath,controllername,writemode,partitiontimelimit,buffertimelimit,partition,nnodes,batchcounter,totmem,ncoresused,scriptlanguage,scriptcommand,scriptflags,scriptext,dbindexes,nthreadsfield,nbatch,nbatcheswrite,docbatches):
    ...
    jobstring+="jobname=\"${modname}_${controllername}_job_${jobnum}\"\n";
    ...
    jobstring+="scriptpath=\"${SLURMONGO_ROOT}/scripts\"\n";
    jobstring+="modulescriptpath=\"${scriptpath}/modules\"\n";
    jobstring+="workpath=\"${SLURMONGO_ROOT}/modules/${modname}/${controllername}/jobs\"\n";
    jobstring+="\n";
    ...
    jobstring+="\n";
    jobstring+="#Main\n";
    jobstring+="outlinenum=1\n";
    jobstring+="stepidcount=0\n";
    jobstring+="statsnum=1\n";
    jobstring+="countready=0\n";
    jobstring+="countdone=0\n";
    jobstring+="usedsteps=0\n";
    jobstring+="usedmem=0\n";
    jobstring+="#Initiate job steps\n";
    jobstring+="for ((stepind=0;stepind<${nsteps};stepind++))\n";
    jobstring+="do\n";
    jobstring+="    stepnum=$((${stepind}+1))\n";
    jobstring+="    stepname=\"${jobname}_step_${stepnum}\"\n";
    jobstring+="    if test -s \"${workpath}/${stepname}.docs\"\n";
    jobstring+="    then\n";
    jobstring+="        stepbatchnum[${stepind}]=1\n";
    jobstring+="        stepbatchname=\"${stepname}_batch_${stepbatchnum[${stepind}]}\"\n";
    jobstring+="        statsname=\"${jobname}_stats_${statsnum}\"\n";
    jobstring+="        statsstepname=\"${statsname}_step_${stepnum}\"\n";
    jobstring+="        statsstepbatchnum[${stepind}]=1\n";
    jobstring+="        statsstepbatchname=\"${statsstepname}_batch_${statsstepbatchnum[${stepind}]}\"\n";
    jobstring+="        head -n ${nbatch} \"${workpath}/${stepname}.docs\" > \"${workpath}/${stepname}.docs.in\"\n";
    jobstring+="        tail -n +$((${nbatch}+1)) \"${workpath}/${stepname}.docs\" > \"${workpath}/${stepname}.docs.temp\"\n";
    jobstring+="        mv \"${workpath}/${stepname}.docs.temp\" \"${workpath}/${stepname}.docs\"\n";

    jobstring+="        echo \"$(date '+%Y.%m.%d:%H.%M.%S'):${jobnum}:$((outlinenum++)): Created ${stepname}.docs.in\"\n";

    jobstring+="        stepmem[${stepind}]=$(echo \"(${totmem}-${usedmem})/(${nsteps}-${countdone}-${usedsteps})\" | bc)\n";
    jobstring+="        mpirun -srun -n \"${nstepthreads[${i}]}\" -J \"${itername}\" --mem-per-cpu=\"${stepmem[${stepind}]}${memunit}\" --time=\"${steptime}\" "+scriptcommandflags+" \"${modulescriptpath}/"+modname+scriptext+"\" \"${workpath}/${stepname}.docs.in\" > \"${workpath}/${stepname}.batch.log\" &\n";
    jobstring+="        sleep 0.5\n";
    jobstring+="        steppids[${stepind}]=$!\n";
    jobstring+="        usedsteps=$((${usedsteps}+1))\n";
    jobstring+="        usedmem=$((${usedmem}+${stepmem[${stepind}]}))\n";
    jobstring+="        stepstatsnum[${stepind}]=0\n";
    jobstring+="        stepstatus[${stepind}]=\"COLLECT\"\n";
    jobstring+="        stepdone[${stepind}]=false\n";
    jobstring+="        stepjobids[${stepind}]=${stepidcount}\n";
    jobstring+="        stepstatsids[${stepind}]=0\n";
    jobstring+="        stepidcount=$((${stepidcount}+1))\n";
    jobstring+="        echo \"$(date '+%Y.%m.%d:%H.%M.%S'):${jobnum}:$((outlinenum++)): Job ${jobnum} Step ${stepnum} Batch ${stepbatchnum[${stepind}]} Started\"\n";
    jobstring+="    else\n";
    jobstring+="        #Remove .docs file, mark current step as done, and reallocate node memory among remaining steps\n";
    jobstring+="        echo \"$(date '+%Y.%m.%d:%H.%M.%S'):${jobnum}:$((outlinenum++)): Job ${jobnum} Step ${stepnum} Complete\"\n";
    jobstring+="        rm \"${workpath}/${stepname}.docs\" 2>/dev/null\n";
    jobstring+="        stepstatus[${stepind}]=\"DONE\"\n";
    jobstring+="        countdone=$((${countdone}+1))\n";
    jobstring+="    fi\n";
    jobstring+="done\n";
    jobstring+="#Loop while any step has unprocessed documents\n";
    jobstring+="while [[ \"${countdone}\" -lt \"${nsteps}\" ]]\n";
    jobstring+="do\n";
    jobstring+="    #Loop over all steps\n";
    jobstring+="    for ((stepind=0;stepind<${nsteps};stepind++))\n";
    jobstring+="    do\n";
    jobstring+="        #Check if a batch process has finished\n";
    jobstring+="        if [ \"${stepstatus[${stepind}]}\" != \"DONE\" ]\n";
    jobstring+="        then\n";
    jobstring+="            if ! kill -0 \"${steppids[${stepind}]}\" 2>/dev/null\n";
    jobstring+="            then\n";
    jobstring+="                usedsteps=$((${usedsteps}-1))\n";
    jobstring+="                usedmem=$((${usedmem}-${stepmem[${stepind}]}))\n";
    jobstring+="                stepnum=$((${stepind}+1))\n";
    ...
    jobstring+="                echo \"$(date '+%Y.%m.%d:%H.%M.%S'):${jobnum}:$((outlinenum++)): Job ${jobnum} Step ${stepnum} stepstatus1: ${stepstatus[${stepind}]}\"\n";

    jobstring+="                if [ \"${stepstatus[${stepind}]}\" = \"COLLECT\" ]\n";
    jobstring+="                then\n";
    jobstring+="                    #Get batch process stats\n";
    jobstring+="                    stepstatarr=($(sacct -n -o 'ExitCode,CPUTimeRAW,MaxRSS,MaxVMSize' -j \"${SLURM_JOBID}.${stepjobids[${stepind}]}\" | sed 's/G/MK/g' | sed 's/M/KK/g' | sed 's/K/000/g' | sed 's/\s\s*/ /g' | cut -d' ' -f1 --complement | head -c -2))\n";
    jobstring+="                    exitcode=\"${stepstatarr[0]}\"\n";
    jobstring+="                    cputime=\"${stepstatarr[1]}\"\n";
    jobstring+="                    maxrss=\"${stepstatarr[2]}\"\n";
    jobstring+="                    maxvmsize=\"${stepstatarr[3]}\"\n";
    ...
    jobstring+="                            elif [[ \"${line}\" =~ ^@.* ]]\n";
    jobstring+="                            then\n";
    jobstring+="                                echo \"${line}\" >> \"${workpath}/${statsname}.stats.temp\"\n";
    jobstring+="                                echo \"CPUTime: $((${cputime}/${nbatch})) seconds\" >> \"${workpath}/${statsname}.stats.temp\"\n";
    jobstring+="                                echo \"MaxRSS: ${maxrss} bytes\" >> \"${workpath}/${statsname}.stats.temp\"\n";
    jobstring+="                                echo \"MaxVMSize: ${maxvmsize} bytes\" >> \"${workpath}/${statsname}.stats.temp\"\n";
    jobstring+="                            else\n";
    jobstring+="                                echo \"${line}\" >> \"${workpath}/${statsname}.stats.temp\"\n";
    jobstring+="                            fi\n";
    jobstring+="                        done < \"${workpath}/${stepname}.batch.log\"\n";
    jobstring+="                    else\n";
    jobstring+="                        skipped=true\n";
    jobstring+="                    fi\n";
    ...
    jobstring+="                    then\n";
    jobstring+="                        #Log the exit code in a .batch.err file\n";
    jobstring+="                        echo \"Stats: ${statsnum}\" >> \"${workpath}/${stepname}.batch.err\"\n";
    jobstring+="                        echo \"ExitCode: ${exitcode}\" >> \"${workpath}/${stepname}.batch.err\"\n";
    jobstring+="                        cat \"${workpath}/${stepname}.batch.log\" >> \"${workpath}/${stepname}.batch.err\"\n";
    jobstring+="                        echo \"\" >> \"${workpath}/${stepname}.batch.err\"\n";
    jobstring+="                        #Restore failed documents to .docs file\n";
    jobstring+="                        cat \"${workpath}/${stepname}.docs.in\" \"${workpath}/${stepname}.docs\" > \"${workpath}/${stepname}.docs.temp\"\n";
    jobstring+="                        mv \"${workpath}/${stepname}.docs.temp\" \"${workpath}/${stepname}.docs\"\n";

    jobstring+="                        echo \"$(date '+%Y.%m.%d:%H.%M.%S'):${jobnum}:$((outlinenum++)): Appended ${statsstepbatchname}.docs.in to .docs after batch fail\"\n";

    jobstring+="                        rm \"${workpath}/${stepname}.docs.in\" 2>/dev/null\n";

    jobstring+="                        echo \"$(date '+%Y.%m.%d:%H.%M.%S'):${jobnum}:$((outlinenum++)): Removed old ${stepname}.docs.in after batch fail\"\n";

    jobstring+="                        echo \"$(date '+%Y.%m.%d:%H.%M.%S'):${jobnum}:$((outlinenum++)): Job ${jobnum} Step ${stepnum} Batch ${stepbatchnum[${stepind}]} Failed\"\n";
    jobstring+="                    else\n";
    jobstring+="                        #Append the .stats.temp file to the .stats.log file and rename .docs.in file to .docs.pend\n";
    jobstring+="                        echo \"$(date '+%Y.%m.%d:%H.%M.%S'):${jobnum}:$((outlinenum++)): Job ${jobnum} Step ${stepnum} Batch ${stepbatchnum[${stepind}]} Completed Successfully\"\n";
    jobstring+="                        cat \"${workpath}/${statsname}.stats.temp\" >> \"${workpath}/${statsname}.stats.log\"\n";
    jobstring+="                        mv \"${workpath}/${stepname}.docs.in\" \"${workpath}/${statsstepbatchname}.docs.pend\"\n";

    jobstring+="                        echo \"$(date '+%Y.%m.%d:%H.%M.%S'):${jobnum}:$((outlinenum++)): Renamed ${stepname}.docs.in to ${statsstepbatchname}.docs.pend after batch success\"\n";

    jobstring+="                        #Check if enough documents are completed\n";
    jobstring+="                        countready=$((${countready}+1))\n";
    jobstring+="                        if [[ \"${countready}\" -eq \"${nbatcheswrite}\" ]] || [[ \"$((${nsteps}-${countdone}))\" -eq 1 ]]\n";
    jobstring+="                        then\n";
    jobstring+="                            #Submit stats process on completed documents\n";
    jobstring+="                            stepmem[${stepind}]=$(echo \"(${totmem}-${usedmem})/(${nsteps}-${countdone}-${usedsteps})\" | bc)\n";
    jobstring+="                            mpirun -srun -n \"${nstepthreads[${stepind}]}\" --exclusive -J \"${statsname}\" --mem-per-cpu=\"${stepmem[${stepind}]}${memunit}\" --time=\"${steptime}\" python \"${scriptpath}/stats.py\" \"${SLURMONGO_ROOT}\" \"${modname}\" \"${controllername}\" \"${statsname}.stats.log\" \"${dbpush}\" \"${markdone}\" \"${writestats}\" >> \"${workpath}/${statsname}.job.log\" &\n";
    jobstring+="                            sleep 0.5\n";
    jobstring+="                            steppids[${stepind}]=$!\n";
    jobstring+="                            stepstatsnum[${stepind}]=${statsnum}\n";
    jobstring+="                            statsnum=$((${statsnum}+1))\n";
    jobstring+="                            stepstatsids[${stepind}]=${stepidcount}\n";
    jobstring+="                            stepidcount=$((${stepidcount}+1))\n";
    jobstring+="                            countready=0\n";
    jobstring+="                            usedsteps=$((${usedsteps}+1))\n";
    jobstring+="                            usedmem=$((${usedmem}+${stepmem[${stepind}]}))\n";
    jobstring+="                            stepstatus[${stepind}]=\"STATS\"\n";
    jobstring+="                            echo \"$(date '+%Y.%m.%d:%H.%M.%S'):${jobnum}:$((outlinenum++)): Job ${jobnum} Step ${stepnum} Stats ${stepstatsnum[${stepind}]} Started\"\n";
    jobstring+="                        else\n";
    jobstring+="                            stepstatus[${stepind}]=\"NEXT\"\n";
    jobstring+="                        fi\n";
    jobstring+="                    fi\n";
    jobstring+="                    #Remove .batch.log and .stats.temp files\n";
    jobstring+="                    rm \"${workpath}/${stepname}.batch.log\" \"${workpath}/${statsname}.stats.temp\" 2>/dev/null\n";
    jobstring+="                elif [ \"${stepstatus[${stepind}]}\" = \"STATS\" ]\n";
    jobstring+="                then\n";
    jobstring+="                    tempstatsname=\"${jobname}_stats_${stepstatsnum[${stepind}]}\"\n";
    jobstring+="                    #Get stats process exit code\n";
    jobstring+="                    exitcode=$(sacct -n -o 'ExitCode' -j \"${SLURM_JOBID}.${stepstatsids[${stepind}]}\" | sed 's/\s\s*/ /g' | cut -d' ' -f2 | head -c -1)\n";
    jobstring+="                    while [[ \"${exitcode}\" == \"\" ]]\n";
    jobstring+="                    do\n";
    jobstring+="                        sleep 0.1\n";
    jobstring+="                        exitcode=$(sacct -n -o 'ExitCode' -j \"${SLURM_JOBID}.${stepstatsids[${stepind}]}\" | sed 's/\s\s*/ /g' | cut -d' ' -f2 | head -c -1)\n";
    jobstring+="                    done\n";
    jobstring+="                    #Check that stats process completed with no errors\n";
    jobstring+="                    skipped=false\n";
    jobstring+="                    if [[ \"${exitcode}\" == \"0:0\" ]] && test -s \"${workpath}/${tempstatsname}.job.log\"\n";
    jobstring+="                    then\n";
    jobstring+="                        while read line\n";
    jobstring+="                        do\n";
    jobstring+="                            if [[ "+" && ".join(["! \"${line}\" =~ ^"+x+".*" for x in outputlinemarkers])+" ]]\n";
    ...
    jobstring+="                                break\n";
    jobstring+="                            fi\n";
    jobstring+="                        done < \"${workpath}/${tempstatsname}.job.log\"\n";
    jobstring+="                    else\n";
    jobstring+="                        skipped=true\n";
    jobstring+="                    fi\n";
    ...
    jobstring+="                    if \"${skipped}\"\n";
    jobstring+="                    then\n";
    jobstring+="                        #Log the exit code in a .stats.err filerestore failed documents to .docs file\n";
    jobstring+="                        echo \"ExitCode: ${exitcode}\" >> \"${workpath}/${tempstatsname}.stats.err\"\n";
    jobstring+="                        cat \"${workpath}/${tempstatsname}.job.log\" >> \"${workpath}/${tempstatsname}.stats.err\"\n";
    jobstring+="                        echo \"\" >> \"${workpath}/${tempstatsname}.stats.err\"\n";
    jobstring+="                        #Restore failed documents to .docs file\n";
    jobstring+="                        tempstatsstepbatches=$(find \"${workpath}/\" -type f -name \"${tempstatsname}_*.docs.pend\" | rev | cut -d'/' -f1 | cut -d'.' -f3 | cut -d'_' -f1,2,3 | rev)\n";
    jobstring+="                        for tempstatsstepbatch in ${tempstatsstepbatches}\n";
    jobstring+="                        do\n";
    jobstring+="                            tempstepind=$(($(echo ${tempstatsstepbatch} | cut -d'_' -f1)-1))\n";
    jobstring+="                            tempstepnum=$((${tempstepind}+1))\n";
    jobstring+="                            tempstepname=\"${jobname}_step_${tempstepnum}\"\n";
    jobstring+="                            tempstatsstepbatchname=\"${tempstatsname}_step_${tempstatsstepbatch}\"\n";
    jobstring+="                            if test -s \"${workpath}/${tempstatsstepbatchname}.docs.pend\"\n";
    jobstring+="                            then\n";
    jobstring+="                                cat \"${workpath}/${tempstatsstepbatchname}.docs.pend\" \"${workpath}/${tempstepname}.docs\" > \"${workpath}/${tempstepname}.docs.temp\"\n";

    jobstring+="                                echo \"$(date '+%Y.%m.%d:%H.%M.%S'):${jobnum}:$((outlinenum++)): Appended ${tempstatsstepbatchname}.docs.pend to .docs after stats fail\"\n";

    jobstring+="                                mv \"${workpath}/${tempstepname}.docs.temp\" \"${workpath}/${tempstepname}.docs\"\n";
    jobstring+="                                rm \"${workpath}/${tempstatsstepbatchname}.docs.pend\" 2>/dev/null\n";

    jobstring+="                                echo \"$(date '+%Y.%m.%d:%H.%M.%S'):${jobnum}:$((outlinenum++)): Removed old ${statsstepbatchname}.docs.pend after stats fail\"\n";

    jobstring+="                                statsstepbatchnum[${tempstepind}]=0\n";
    jobstring+="                                stepstatus[${tempstepind}]=\"NEXT\"\n";
    jobstring+="                            fi\n";
    jobstring+="                        done\n";
    jobstring+="                        echo \"$(date '+%Y.%m.%d:%H.%M.%S'):${jobnum}:$((outlinenum++)): Job ${jobnum} Step ${stepnum} Stats ${stepstatsnum[${stepind}]} Failed\"\n";
    jobstring+="                    else\n";
    jobstring+="                        #Remove all .docs.pend files from completed steps\n";
    jobstring+="                        tempstatsstepbatches=$(find \"${workpath}/\" -type f -name \"${tempstatsname}_*.docs.pend\" | rev | cut -d'/' -f1 | cut -d'.' -f3 | cut -d'_' -f1,2,3 | rev)\n";
    jobstring+="                        for tempstatsstepbatch in ${tempstatsstepbatches}\n";
    jobstring+="                        do\n";
    jobstring+="                            tempstepind=$(($(echo ${tempstatsstepbatch} | cut -d'_' -f1)-1))\n";
    ...
    jobstring+="                                echo \"$(date '+%Y.%m.%d:%H.%M.%S'):${jobnum}:$((outlinenum++)): Removed ${tempstatsstepbatchname}.docs.pend after stats success\"\n";

    jobstring+="                                statsstepbatchnum[${tempstepind}]=0\n";
    jobstring+="                                stepstatus[${tempstepind}]=\"NEXT\"\n";
    jobstring+="                            fi\n";
    jobstring+="                        done\n";
    jobstring+="                        echo \"$(date '+%Y.%m.%d:%H.%M.%S'):${jobnum}:$((outlinenum++)): Job ${jobnum} Step ${stepnum} Stats ${stepstatsnum[${stepind}]} Completed Successfully\"\n";
    jobstring+="                    fi\n";
    jobstring+="                    rm \"${workpath}/${tempstatsname}.stats.log\" 2>/dev/null\n";
    jobstring+="                    stepstatsnum[${stepind}]=0\n";
    jobstring+="                    stepstatus[${stepind}]=\"NEXT\"\n";
    jobstring+="                fi\n";
    ...
    jobstring+="                echo \"$(date '+%Y.%m.%d:%H.%M.%S'):${jobnum}:$((outlinenum++)): Job ${jobnum} Step ${stepnum} stepstatus2: ${stepstatus[${stepind}]}\"\n";

    jobstring+="                if [ \"${stepstatus[${stepind}]}\" = \"NEXT\" ]\n";
    jobstring+="                then\n";
    jobstring+="                    #Check if any documents are left in queue\n";
    jobstring+="                    if test -s \"${workpath}/${stepname}.docs\"\n";
    jobstring+="                    then\n";
    jobstring+="                        #Submit next batch process on current step\n";
    jobstring+="                        stepbatchnum[${stepind}]=$((${stepbatchnum[${stepind}]}+1))\n";
    jobstring+="                        stepbatchname=\"${stepname}_batch_${stepbatchnum[${stepind}]}\"\n";
    jobstring+="                        statsstepbatchnum[${stepind}]=$((${statsstepbatchnum[${stepind}]}+1))\n";
    jobstring+="                        statsstepbatchname=\"${statsstepname}_batch_${statsstepbatchnum[${stepind}]}\"\n";
    jobstring+="                        head -n ${nbatch} \"${workpath}/${stepname}.docs\" > \"${workpath}/${stepname}.docs.in\"\n";
    jobstring+="                        tail -n +$((${nbatch}+1)) \"${workpath}/${stepname}.docs\" > \"${workpath}/${stepname}.docs.temp\"\n";
    jobstring+="                        mv \"${workpath}/${stepname}.docs.temp\" \"${workpath}/${stepname}.docs\"\n";
...
try:
    ...
    #batchcounter=1;
    #stepcounter=1;
    while (prevcontrollersrunningq(username,prevmodlist,controllername) or userjobsrunningq(username,modname,controllername) or firstlastrun) and ((querylimit==None) or (stepcounter<=querylimit)) and (timeleft(starttime,controllerbuffertimelimit)>0):

...
def islimitreached(controllerpath,querylimit):
    if querylimit==None:
        ...
        #if niters==1:
        #    ntot=eval(subprocess.Popen("echo \"$(cat "+controllerpath+"/jobs/*.stat 2>/dev/null | wc -l)+$(cat "+controllerpath+"/jobs/*.log 2>/dev/null | grep 'CPUTime' | wc -l)\" | bc | head -c -1",shell=True,stdout=subprocess.PIPE,preexec_fn=default_sigpipe).communicate()[0]);
        #elif niters>1:
        ntot=eval(subprocess.Popen("echo \"$(cat $(find "+controllerpath+"/jobs/ -type f -name '*.docs' 2>/dev/null) 2>/dev/null | wc -l)+$(cat "+controllerpath+"/jobs/*.log 2>/dev/null | grep 'CPUTime' | wc -l)\" | bc | head -c -1",shell=True,stdout=subprocess.PIPE,preexec_fn=default_sigpipe).communicate()[0]);
            ...
            for line in skippedstream:
                ...
                if eval(resubmitq):
                    ...
                    if skippedjobsplit[:2]==[modname,controllername]:
                        ...
                        os.remove(controllerpath+"/jobs/"+skippedjob+".stat");
                        os.remove(controllerpath+"/jobs/"+skippedjob+".docs");
                        os.remove(controllerpath+"/jobs/"+skippedjob+".temp");
                    else:
...
def writejobfile(modname,dbpush,markdone,writestats,writestorage,jobname,jobstepnames,controllerpath,controllername,writemode,partitiontimelimit,buffertimelimit,partition,nnodes,batchcounter,totmem,ncoresused,scriptlanguage,scriptcommand,scriptflags,scriptext,dbindexes,nthreadsfield,nwrite,docbatches):
    nbatches=len(docbatches);
    ...
    jobstring+="for ((i=0;i<${nsteps};i++))\n";
    jobstring+="do\n";
    jobstring+="    stepnum=$(($i+1))\n";
    jobstring+="    stepname=\"${jobname}_step_${stepnum}\"\n";
    jobstring+="    steptypes[${i}]=\"JOB\"\n";
    jobstring+="    stepjobiters[${i}]=1\n";
    jobstring+="    stepstatsiters[${i}]=0\n";
    jobstring+="    itername=\"${stepname}_iter_${stepjobiters[${i}]}\"\n";
    jobstring+="    mpirun -srun -n \"${nstepthreads[${i}]}\" -J \"${itername}\" --mem-per-cpu=\"${stepmem}${memunit}\" --time=\"${steptime}\" "+scriptcommandflags+" \"${modulescriptpath}/"+modname+scriptext+"\" \"${workpath}/${stepname}.docs\" \"${nwrite}\" > \"${workpath}/${stepname}.temp\" &\n";
    jobstring+="    sleep 0.1\n";
    jobstring+="    steppids[${i}]=$!\n";
    jobstring+="    stepjobids[${i}]=${stepidcount}\n";
    ...
    jobstring+="    for ((i=0;i<${nsteps};i++))\n";
    jobstring+="    do\n";
    jobstring+="        if ! kill -0 \"${steppids[${i}]}\" 2>/dev/null\n";
    jobstring+="        then\n";
    jobstring+="            stepnum=$(($i+1))\n";
    jobstring+="            stepname=\"${jobname}_step_${stepnum}\"\n";
    jobstring+="            if [ \"${steptypes[${i}]}\" == \"JOB\" ]\n";
    jobstring+="            then\n";
    jobstring+="                jobstepstats=($(sacct -n -o 'ExitCode,CPUTimeRAW,MaxRSS,MaxVMSize' -j \"${SLURM_JOBID}.${stepjobids[${i}]}\" | sed 's/G/MK/g' | sed 's/M/KK/g' | sed 's/K/000/g' | sed 's/\s\s*/ /g' | cut -d' ' -f1 --complement | head -c -2))\n";
    ...
    jobstring+="                            break\n";
    jobstring+="                        fi\n";
    jobstring+="                    done < \"${workpath}/${stepname}.temp\"\n";
    jobstring+="                else\n";
    jobstring+="                    skipped=true\n";
    jobstring+="                fi\n";
    jobstring+="                steptypes[${i}]=\"STATS\"\n";
    jobstring+="                stepstatsiters[${i}]=$((${stepstatsiters[${i}]}+1))\n";
    jobstring+="                itername=\"stats_${stepname}_iter_${stepstatsiters[${i}]}\"\n";
    jobstring+="                if \"${skipped}\"\n";
    jobstring+="                then\n";
    jobstring+="                    echo \"${stepname},${exitcode},False\" > \"${workpath}/${stepname}.stat\"\n";
    jobstring+="                else\n";
    jobstring+="                    mpirun -srun -n \"${nstepthreads[${i}]}\" --exclusive -J \"${itername}\" --mem-per-cpu=\"${stepmem}${memunit}\" --time=\"${steptime}\" python \"${scriptpath}/stats.py\" \"${SLURMONGO_ROOT}\" \"${modname}\" \"${controllername}\" \"${jobnum}\" \"${stepnum}\" \"${nwrite}\" \"${dbpush}\" \"${markdone}\" \"${writestats}\" \"${writestorage}\" \"${cputime}\" \"${maxrss}\" \"${maxvmsize}\" >> \"${workpath}/${stepname}.log\" &\n";
    jobstring+="                    sleep 0.1\n";
    jobstring+="                    steppids[${i}]=$!\n";
    jobstring+="                    stepidcount=$((${stepidcount}+1))\n";
    jobstring+="                fi\n";
    jobstring+="            elif [ \"${steptypes[${i}]}\" == \"STATS\" ]\n";
    jobstring+="            then\n";
    jobstring+="                if test -f \"${workpath}/${stepname}.docs\"\n";
    jobstring+="                then\n";
    jobstring+="                    steptypes[${i}]=\"JOB\"\n";
    jobstring+="                    stepjobiters[${i}]=$((${stepjobiters[${i}]}+1))\n";
    jobstring+="                    itername=\"${stepname}_iter_${stepjobiters[${i}]}\"\n";
    jobstring+="                    mpirun -srun -n \"${nstepthreads[${i}]}\" -J \"${itername}\" --mem-per-cpu=\"${stepmem}${memunit}\" --time=\"${steptime}\" "+scriptcommandflags+" \"${modulescriptpath}/"+modname+scriptext+"\" \"${workpath}/${stepname}.docs\" \"${nwrite}\" >> \"${workpath}/${stepname}.temp\" &\n";
    jobstring+="                    sleep 0.1\n";
    jobstring+="                    steppids[${i}]=$!\n";
    ...
    jobstring+="                    stepidcount=$((${stepidcount}+1))\n";
    jobstring+="                else\n";
    jobstring+="                    steptypes[${i}]=\"DONE\"\n";
    jobstring+="                    countdone=$((${countdone}+1))\n";
    jobstring+="                    stepmem=$((${totmem}/(${nsteps}-${countdone})))\n";
    jobstring+="                fi\n";
    jobstring+="            fi\n";
    jobstring+="        fi\n";
    jobstring+="    done\n";
...
def doaction(batchcounter,stepcounter,inputdoc,docbatch,nthreadsfield,licensestream,username,maxjobcount,modlist,modulesdirpath,softwarestatefile,scriptpath,scriptlanguage,starttime,controllerbuffertimelimit,statusstatefile,sleeptime,partitions,resourcesstatefile,scriptmemorylimit,maxstepcount,modname,controllername,dbindexes,dbpush,markdone,writestats,writestorage,controllerpath,writemode,mongouri,scriptcommand,scriptflags,scriptext,querystatefilename,basecollection,niters,nwrite):
    ...
    writejobfile(modname,dbpush,markdone,writestats,writestorage,jobname,jobstepnames,controllerpath,controllername,writemode,partitiontimelimit,buffertimelimit,partition,nnodes,batchcounter,totmem,ncoresused,scriptlanguage,scriptcommand,scriptflags,scriptext,dbindexes,nthreadsfield,nwrite,docbatchwrite);
    #Submit job file
    if nthreadsfield!="":
...
try:
    ...
    #Options
    blocking=eval(sys.argv[16]);
    dbpush=sys.argv[17];
    markdone=sys.argv[18];
    writestats=sys.argv[19];
    writestorage=sys.argv[20];
    niters=eval(sys.argv[21]);
    nwrite=eval(sys.argv[22]);
